...
Pokemon* load_pokemon_list(const char* filename, int* outCount)
{
    ...
    while (!feof(file))
    {
        ...
        int read = fscanf_s(file,
            "%7s %31s %19s %19s %d %d %d %d %d %d %19s %19s %19s %19s\n",
            p.idnumber, (unsigned)_countof(p.idnumber),  // Read ID number string and buffer size
            p.name, (unsigned)_countof(p.name),          // Read Pokemon name string and buffer size
            type1, (unsigned)sizeof(type1),              // Read first type string and buffer size
            type2, (unsigned)sizeof(type2),              // Read second type string and buffer size
            &p.hp, &p.attack, &p.defense, &p.spAttack, &p.spDefence, &p.speed, // Read integer stats
            p.moves[0], (unsigned)_countof(p.moves[0]),  // Read move names and their buffer sizes (4 moves)
            p.moves[1], (unsigned)_countof(p.moves[1]),
            p.moves[2], (unsigned)_countof(p.moves[2]),
            p.moves[3], (unsigned)_countof(p.moves[3]));
...
void printPokemon(const char* name)
{
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        strcpy_s(spriteBuffer + used, MAX_SPRITE_SIZE - used, line);
        used += len;
    }
...
char* loadPokemonSpriteByID(const char* id) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        strcpy_s(buffer + used, MAX_SPRITE_SIZE - used, line);
        used += len;
    }
...
void cleanup_orphaned_custom_sprites()
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf_s(line, "%15s", id, (unsigned)_countof(id)) == 1) 
        {
    ...
    struct _finddata_t file; // Holds info about each found file
    char searchPath[128];
    snprintf(searchPath, sizeof(searchPath), "%s*.txt", CUSTOM_SPRITE_FOLDER); // Look for all .txt files

    intptr_t searchHandle = _findfirst(searchPath, &file); // Start searching files
    if (searchHandle == -1) {
        printf("No custom sprites to clean up.\n");
        goto cleanup; // No files found, skip to cleanup
    }

    // Step 3: Iterate over all found files
    while (1) {
        char* dot = strrchr(file.name, '.'); // Find last '.' in filename
        if (dot) *dot = '\0';                 // Remove file extension (".txt")

        // Only process files starting with 'C' (custom Pokemon sprites)
        if (file.name[0] == 'C') {
            bool found = false;

            // Check if the current file's ID is in the valid IDs list
            for (int i = 0; i < validCount; i++) {
                if (strcmp(validIDs[i], file.name) == 0) {
                    found = true; // Found valid Pokemon ID
                    break;
                }
            }

            // If not found in valid IDs, delete the orphaned sprite file
            if (!found) {
                char fullPath[256];
                snprintf(fullPath, sizeof(fullPath), "%s%s.txt", CUSTOM_SPRITE_FOLDER, file.name);
                if (remove(fullPath) == 0) {
                    printf("Deleted orphaned sprite: %s\n", file.name);
                }
                else {
        ...
        // Move to the next file; break if no more files are found
        if (_findnext(searchHandle, &file) != 0)
            break;
    }

    _findclose(searchHandle); // Close the directory search handle to free system resources
...
Move* load_move_list_dynamic(const char* filename, int* outCount)
{
    ...
    while (fgets(raw, sizeof(raw), file))
    {
        ...
        m->power = power;
        strcpy_s(m->accuracy, sizeof(m->accuracy), accStr);
        strcpy_s(m->statTarget, sizeof(m->statTarget), statTarget);
        m->statChange = statChange;
        strcpy_s(m->statusEffect, sizeof(m->statusEffect), statusEffect);
    }
