...
#include <string.h>
#include <stdbool.h>
#include "platform.h" /* cross-platform */
#include <ctype.h>    // for tolower, toupper

static int display_list_menu(const char* title, char** items, int count);
...
int load_category_list(char*** outFileList, int* outCount)
{
    ...
    char searchPath[128];
    snprintf(searchPath, sizeof(searchPath), "%s*", SPRITE_ASSET_FOLDER);
    intptr_t handle = _findfirst(searchPath, &file);
    if (handle == -1) { *outFileList = NULL; *outCount = 0; return 0; }
    ...
    while (result == 0) {
        if ((file.attrib & _A_SUBDIR) && strcmp(file.name, ".") != 0 && strcmp(file.name, "..") != 0) {
            if (count >= capacity) { capacity *= 2; list = realloc(list, capacity * sizeof(char*)); }
    ...
    _findclose(handle);
#else
    DIR* dir = opendir(SPRITE_ASSET_FOLDER);
    if (!dir) { *outFileList = NULL; *outCount = 0; return 0; }
    int capacity = 10, count = 0;
    char** list = malloc(capacity * sizeof(char*));
    struct dirent* entry;
...
int load_creature_list(const char* category, char*** outFileList, int* outCount)
{
    ...
    if (!fileList) { _findclose(handle); return 0; }
    int result = 0;
    while (result == 0) {
        if (count >= capacity) { capacity *= 2; fileList = realloc(fileList, capacity * sizeof(char*)); if (!fileList) { _findclose(handle); return 0; } }
...
#else
    char dirPath[128];
    snprintf(dirPath, sizeof(dirPath), "%s%s", SPRITE_ASSET_FOLDER, category);
    DIR* dir = opendir(dirPath);
    if (!dir) { *outFileList = NULL; *outCount = 0; return 0; }
    int capacity = 10, count = 0;
    char** fileList = malloc(capacity * sizeof(char*));
    if (!fileList) { closedir(dir); return 0; }
    struct dirent* entry;
    while ((entry = readdir(dir)) != NULL) {
        size_t nlen = strlen(entry->d_name);
        if (nlen > 4 && strcmp(entry->d_name + nlen - 4, ".txt") == 0) {
            if (count >= capacity) { capacity *= 2; fileList = realloc(fileList, capacity * sizeof(char*)); if (!fileList) { closedir(dir); return 0; } }
